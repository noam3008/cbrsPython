...
class LoggerHandler(object):
    ...
    def create_New_Logger(self,logger_name, level=logging.INFO):
        log_file =  '\Logs\session_' + str(datetime.now().strftime("%Y_%m_%d_%H_%M_%S")) +'.log'
    ...
    def print_And_Log_To_File(self,nameOfLogger,msg, level="info"):       
    ...
    def get_Logger_By_Test_Number(self,nameOfLogger):
        log = logging.getLogger(nameOfLogger)
        return log